...
public class EgovDateUtilTest {
	 ...
	 * 특정 날짜를 인자로 받아 그 일자로부터 주어진 기간만큼 추가한 날 계산 테스트
	 * @throws Exception
	 */
	@Test
	...
	public void testCurrentDate() throws Exception {
		...
		log.debug(EgovDateUtil.getCurrentMilliSecondAsInt());

		
		// type integer
		assertEquals(EgovStringUtil.lPad(String.valueOf(cal.get(Calendar.YEAR)), 4, '0'), 
				EgovDateUtil.getCurrentYearAsString());

		assertEquals(EgovStringUtil.lPad(String.valueOf(cal.get(Calendar.MONTH) + 1), 2, '0'), 
				EgovDateUtil.getCurrentMonthAsString());
		
		assertEquals(EgovStringUtil.lPad(String.valueOf(cal.get(Calendar.DAY_OF_MONTH)), 2, '0'), 
				EgovDateUtil.getCurrentDayAsString());
		
		assertEquals(EgovStringUtil.lPad(String.valueOf(cal.get(Calendar.HOUR_OF_DAY)), 2, '0'), 
				EgovDateUtil.getCurrentHourAsString());
		
		assertEquals(EgovStringUtil.lPad(String.valueOf(cal.get(Calendar.MINUTE)), 2, '0'), 
				EgovDateUtil.getCurrentMinuteAsString());
		
		log.debug(EgovDateUtil.getCurrentSecondAsString());

		log.debug(EgovDateUtil.getCurrentMilliSecondAsString());
		...
		log.debug(EgovDateUtil.getCurrentTimeAsString());
		
		log.debug(EgovDateUtil.getCurrentDateTimeAsString());
		
		
	}
	
	/**
	 * [Flow #-3] Positive Case : 
	 * 해당 일자의 요일을 조회한다.
	 * @throws Exception
	 */
	@Test
	...
	public void testDateUtilEtc() throws Exception {
		...
		log.debug(EgovDateUtil.getCurrentFullAge("7701011234567"));
	...
	public void testDayCount() throws Exception {
		...
		try {
			log.debug(EgovDateUtil.getDayCountWithFormatter("2008/12/01", "2009/06/01", "yyyyMMdd"));
		} catch (Exception e) {
	...
	public void testTimeCount() throws Exception {
		...
		Class<Exception> exceptionClass = null;
		try {
			log.debug(EgovDateUtil.getTimeCount("200903010000", "20090301000100"));
		} catch (Exception e) {
			log.error("### Exception : " + e.toString());
			exceptionClass = (Class<Exception>) e.getClass();
		} finally {
	...
	/**
	 * [Flow #-8] Positive Case : 
	 * 해당 문자열이 주어진 일자 형식을 준수하는지의 여부와 존재하는 날짜인지를 검사한다.
	 * @throws Exception
	 */
	@Test
