 ...
 * <p>
 * <b>NOTE:</b> 전자정부 연계 서비스의 표준 메시지의 Type을 정의하기 위한 기본
 * Class이다. 메시지를 형식(Type)을 정의하는 Class 의 최상위 Class 이다.
 * @author 실행환경 개발팀 심상호
 * @since 2009.06.01
 * @version 1.0
 * @see <pre>
...
public abstract class AbstractType implements Type {
    ...
    protected AbstractType(final String id, final String name,
        ...
        if (StringUtils.hasText(id) == false) {
            LOG.error("id does not have a text (id = \"" + id + "\")");
            throw new IllegalArgumentException();
        } else if (StringUtils.hasText(name) == false) {
            LOG.error("name does not have a text (name = \"" + name + "\")");
            throw new IllegalArgumentException();
        } else if (assignableClasses == null || assignableClasses.size() == 0) {
            LOG
                .error("assignableClasses is null or empty (assignableClasses = "
                    + assignableClasses + ")");
            throw new IllegalArgumentException();
        }
        this.id = id;
     ...
     * @throws IllegalArgumentException
     *         Argument <code>assignableClasses</code>
     *         가 <code>null</code>이거나 빈 Array인 경우
     */
    ...
    public boolean isAssignableFrom(final Class<?> clazz) {
        if (clazz == null) {
            LOG.error("Argument 'clazz' is null");
            throw new IllegalArgumentException();
        }
