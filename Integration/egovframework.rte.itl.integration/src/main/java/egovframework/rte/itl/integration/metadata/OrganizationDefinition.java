 ...
 * <p>
 * <b>NOTE:</b> 전자정부 연계 서비스 메타 데이터 중 '기관'를 나타내는
 * class이다.
 * @author 실행환경 개발팀 심상호
 * @since 2009.06.01
 * @version 1.0
...
public class OrganizationDefinition implements Validatable {
    ...
    /** 소속 시스템 */
    private Map<String, SystemDefinition> systems =
        new HashMap<String, SystemDefinition>();
    ...
    public boolean isValid() {
        if (statusChanged.getAndSet(false)) {
            valid =
                (StringUtils.hasText(id) && StringUtils.hasText(name) && systems != null);
            if (systems != null) {
