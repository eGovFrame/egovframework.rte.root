 ...
 * <p>
 * <b>NOTE:</b> 전자정부 연계 서비스 메타 데이터 중 '서비스'를 정의하기 위한
 * Class이다. <br>
 * 모든 서비스를 고유한 <code>id</code>를 가지고 있으며, 반드시 요청 메시지
 * Type Id (<code>requestMessageTypeId</code>)와 응답 메시지
 * Type Id (<code>responseMessageTypeId</code>)를 가져야
 * 한다. Attribute <code>serviceProviderBeanId</code>는
 * 서비스를 제공하는 객체의 id로 Spring Framework의 Application
 * Context에 등록되어 있는 bean id이다. 만약 <code>null</code> 값인
 * 경우 외부에서 제공받는 서비스를 의미한다.
 * @author 실행환경 개발팀 심상호
 * @since 2009.06.01
 * @version 1.0
 * @see <pre>
...
public class ServiceDefinition implements Validatable {
    ...
    public boolean isValid() {
        if (statusChanged.getAndSet(false)) {
            valid =
                (StringUtils.hasText(key) && system != null
    ...
    public String toString() {
        ...
        sb.append("\n\tid = ").append(StringUtils.quote(id)).append(
            ...
            "\n\trequestMessageTypeId = ").append(
            StringUtils.quote(requestMessageTypeId)).append(
            "\n\tresponseMessageTypeId = ").append(
            StringUtils.quote(responseMessageTypeId)).append(
            "\n\tserivceProviderBeanId = ").append(
            StringUtils.quote(serviceProviderBeanId)).append("\n\tusing = ")
