...
/**
 * 연계 서비스 메타 데이터의 IntegrationDefinition을 읽어오기 위한 DAO
 * 인터페이스
 * <p>
 * <b>NOTE:</b> 전자정부 연계 서비스 메타 데이터의
 * IntegrationDefinition을 읽어오기 위한 Data Access Object
 * interface이다.
 * @author 실행환경 개발팀 심상호
 * @since 2009.06.01
 * @version 1.0
...
public interface IntegrationDefinitionDao {
    ...
    /**
     * 연계 primary key (<code>id</code>)에 해당하는
     * IntegrationDefinition을 읽어온다.
     * @param id
     *        연계 ID
     * @return IntegrationDefinition, 해당하는 값이 없를 경우
     *         <code>null</code>
     */
    public IntegrationDefinition getIntegrationDefinition(final String id);

    /**
     * 시스템의 alternative key (
     * <code>consumerOrganizationId</code>,
     * <code>consumerSystemId</code>)이 요청 시스템으로 등록되어 있는
     * IntegrationDefinition을 읽어온다.
    ...
    public List<IntegrationDefinition> getIntegrationDefinitionOfConsumer(
            final String consumerOrganizationId, final String consumerSystemId);

    /**
     * 시스템의 alternative key (
     * <code>providerOrganizationId</code>,
     * <code>providerSystemId</code>)이 제공 시스템으로 등록되어 있는
     * IntegrationDefinition을 읽어온다.
    ...
    public List<IntegrationDefinition> getIntegrationDefinitionOfProvider(
            final String providerOrganizationId, final String providerSystemId);
}
